...
?>
	<div id="home_left">
		<div id="cat_tit" style="float:left;height:25px; width:630px; margin-bottom:10px; border-bottom:1px dashed #CCCCCC; color:#ff0800;font-size:18px;font-weight:bold;"><?=$str_name_cat?></div>
		<div id="cat_featured" style="margin-top:2px; margin-bottom:10px; width:650px;float:left;clear:both;">
		<?php 
			if($rs_cat_news)
			{
				$first_news = $rs_cat_news->row(0);
				$news_img = empty($first_news->news_img) ? _random_image() : $first_news->news_img;
				$image = '<img src="'.$url.$news_img.'" width="357" alt="'.str_replace('"', "'", stripslashes($first_news->news_quickview)).'" />';
		echo '<div style="float:left; width:362px; height:222px;overflow:hidden; margin-right:10px;">
				<a href="'.site_url('news/'.$first_news->id_cattext.'/'.$first_news->id_text).'" title="'.stripslashes($first_news->news_quickview).'">'.$image.'</a>
			</div>
			<div style="float:right; width:270px;">
				<h2 style="margin-bottom:10px;"><a  href="'.site_url('news/'.$first_news->id_cattext.'/'.$first_news->id_text).'" title="'.stripslashes($first_news->news_quickview).'">'.word_limiter(stripslashes($first_news->news_title), 14).'</a></h2>
				<p style="color:#5d5d5d">'.word_limiter(stripslashes($first_news->news_quickview), 200).' <a href="'.site_url('news/'.$first_news->id_cattext.'/'.$first_news->id_text).'"><strong>Xem tiếp</strong></a> &raquo;</p>
			</div>';
			}
			else
			{
		...
		<div id="c4"  style="width:660px;height:90px;overflow:hidden" class="qcao marbottom5 martop5" align="center"><?=showAd($this->config->item('qc_c4'));?></div>
		<div id="cat_items_left" style="width:500px;float:left">
		<?php 
			if($total_cat_news>1)
			{
				for($i=1;$i<$total_cat_news;$i++)
				{
					$first_news = $rs_cat_news->row($i);
					$news_img = empty($first_news->news_img_thumb)?_random_image():$first_news->news_img_thumb;
					$image = showIMG($news_img, 144, 97, '', '', '', 0, 'style="margin-right:10px; border:1px solid #F2F2F2"');
		echo '<div id="cat_item_left">
				...
				<div style="float:right;width:310px">
					<p><a href="'.site_url('news/'.$first_news->id_cattext.'/'.$first_news->id_text).'" style="font-size:14px;font-weight: bold;">'.word_limiter(stripslashes($first_news->news_title), 22).'</a></p>
					<h6>'.date('d/m/Y', $first_news->news_date).'</h6>
					<p>'.word_limiter(stripslashes($first_news->news_quickview), 31).'</p>
				</div>
			</div>';
				}
			}
		...
		<div id="cat_bot_list" style="width:500px">
			<p style="font-weight:bold; color:#FF0000">TIN KHÁC</p>
			<ul>
		<?php 
			if($total_other_cat_news)
			{
				for($i=0;$i<$total_other_cat_news;$i++)
				{
					$first_news = $rs_other_cat_news->row($i);
					echo '<li><a href="'.site_url('news/'.$first_news->id_cattext.'/'.$first_news->id_text).'">'.word_limiter(stripslashes($first_news->news_title), 14).'</a></li>';
				}
			}
		?>
...
<script type="text/javascript" language="javascript">
swapAd('c1');swapAd('c2');swapAd('c3');swapAd('c4');swapAd('c5');swapAd('c6');swapAd('c7');swapAd('c8');
</script>